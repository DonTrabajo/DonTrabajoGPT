...
def run():
    ...
    if not findings:
        console.print("[yellow]No artifacts found in $ARTIFACTS_DIR. Paste findings, then press Ctrl-D when done.[/]")
        findings = sys.stdin.read()

    sys_path = _env("DONTRABAJO_SYSTEM","tools/oss_persona/persona_prompt.txt")
    if Path(sys_path).exists():
        system = Path(sys_path).read_text(encoding="utf-8", errors="ignore")
    else:
        system = "You are DonTrabajoGPT offline. Be concise, OPSEC-safe, and practical."
